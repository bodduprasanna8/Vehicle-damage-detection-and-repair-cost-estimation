...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'gfbgvjnfg'
...
def draw_bounding_boxes(image, results, class_names):
    """
    Draw bounding boxes and labels on an image using Yolov5 results with a single color for all labels.
    ...
    """
    # Define a single color for all bounding boxes (e.g., Blue)
    color = (0, 0, 255)  # Red in BGR format (you can change this to any color)

    # Convert PIL Image to OpenCV format if needed
    if not isinstance(image, np.ndarray):
        image = np.array(image)[:, :, ::-1]  # Convert RGB (PIL) to BGR (OpenCV)
    ...
    for box, cls in zip(results[0].boxes.xyxy, results[0].boxes.cls):
        ...
        # Ensure text fits within the image
...
def upload():
    if request.method == 'POST':
        ...
        for detected_class in detected_classes:
            damage_detected = class_names[detected_class]  # Damage type
        ...
        # Pass damage details and total cost to the template
